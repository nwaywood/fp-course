...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  -- (<$>) f (P fn) = P (\input -> case fn input of
...
(|||) (P fn1) (P fn2) =
    ...
    --     UnexpectedString s -> UnexpectedString s
    --     ExpectedEof i -> ExpectedEof i)
    -- P (\input -> bool (fn1 input) (fn2 input) (isErrorResult (fn1 input)))
    -- P (\input -> bool (fn1 input) (fn2 input) ((isErrorResult . fn1) input))
    -- P (lift3 bool fn1 fn2 (isErrorResult . fn1))
    P (\input -> let r = fn1 input in bool r (fn2 input) (isErrorResult r))
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser b
  (=<<) aToPB (P fn) = P (\input ->
    onResult (fn input) (\i x -> parse (aToPB x) i))
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  -- (<*>) (P aToB) (P fn) = P (\input -> onResult (fn input) (\i a -> _))
  (<*>) pf pa =
      pf >>= \f ->
      pa >>= \a ->
      pure (f a)
...
(.:.) ::
  ...
  -> Parser (List a)
  -> Parser (List a)
-- teaches cons how to work inside "Parser"
(.:.) = lift2 (:.)
